 ...
 * @return boolean
 */
function local_exhibit_builder_is_current_exhibit($exhibit)
{
 ...
 * @return string
 */
function local_exhibit_builder_link_to_exhibit($exhibit = null, $text = null, $props = array(), $exhibitPage = null)
    {
    if (!$exhibit) 
        {
        $exhibit = get_current_record('exhibit');
        }
    $uri = local_exhibit_builder_exhibit_uri($exhibit, $exhibitPage);
 ...
 * @return string
 */
function local_exhibit_builder_exhibit_uri($exhibit = null, $exhibitPage = null)
    {
    if (!$exhibit) 
        {
        $exhibit = get_current_record('exhibit');
...
//  override default URIs to redirect for exhibits which are linked on the homepage but which live on other servers - 20201022, jss
        if (preg_match('/student-theatre/', $uri))
            {
            $uri = "https://projects.lib.rochester.edu";
            }
 ...
 * @return string
 */
function local_exhibit_builder_link_to_exhibit_item($text = null, $props = array(), $item = null)
{
    if (!$item) {
 ...
 * @return string
 */
function local_exhibit_builder_exhibit_item_uri($item, $exhibit = null)
{
    if (!$exhibit) {
        $exhibit = get_current_record('exhibit');
    }
 ...
 * @return array
 */
function local_exhibit_builder_recent_exhibits($num = 10)
{
 ...
 * @return array
 */
function local_exhibit_builder_get_themes()
{
    $themeNames = array();
 ...
 * @return string
 */
function local_exhibit_builder_display_random_featured_exhibit()
{
 ...
 * @return Exhibit|null
 */
function local_exhibit_builder_random_featured_exhibit()
{
...
* @return string
*/
function local_link_to_exhibit($text = null, $props = array(), $exhibitPage = null, $exhibit = null)
{
    return local_exhibit_builder_link_to_exhibit($exhibit, $text, $props, $exhibitPage);
}
 ...
 * @return string
 */
function local_exhibit_builder_exhibits_shortcode($args, $view)
{
    $params = array();
 ...
 * @return string
 */
function local_exhibit_builder_featured_exhibits_shortcode($args, $view)
{
    $args['is_featured'] = 1;
